...
export default function ProgramUpdateModal({ onClose, selectedProgram }: { onClose: () => void, selectedProgram: IProgramData }) {
    const { register, handleSubmit, reset, formState: { errors } } = useForm<TUpdateProgramInfo>({
        defaultValues: {
            name: selectedProgram.name,
            trainingLevel: selectedProgram.trainingLevel as "COLLEGE" | "BACHELOR" | "MASTER",
            educationType: selectedProgram.educationType as "FULL_TIME" | "PART_TIME" | "DISTANCE_LEARNING",
            plannedEducationYear: selectedProgram.plannedEducationYear,
            feePerCredit: selectedProgram.feePerCredit,
            version: selectedProgram.version,
            majorId: selectedProgram.major.id
        },
        mode: "onBlur"
    });
    const { data: majorData, isLoading: isLoadingMajor } = useGetMajorsSimple();
    const mutation = useUpdateProgramInfo(onClose);
    const onSubmit = (data: TUpdateProgramInfo) => {
        mutation.mutate({
            programId: selectedProgram.id, data: {
                name: data.name,
                trainingLevel: data.trainingLevel,
                educationType: data.educationType,
                plannedEducationYear: data.plannedEducationYear,
