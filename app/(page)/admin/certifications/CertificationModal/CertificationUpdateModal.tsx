...
export default function CertificationUpdateModal({ onClose, selectedCertification }: { onClose: () => void, selectedCertification: ICertificationData }) {
    const { register, handleSubmit, reset, formState: { errors } } = useForm<TUpdateCertificationInfo>({
        defaultValues: {
    ...
    const mutation = useUpdateCertificationInfo(onClose);
    const onSubmit = (data: TUpdateCertificationInfo) => {
        mutation.mutate({
            certificateId: selectedCertification.id, data: {
                name: data.name,
