        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-4">
            <div className="flex items-center gap-3 mb-3">
                ...
                <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{data.lecturerName}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{data.lecturerCode}</p>
                </div>
            </div>
            <div className="border-t border-gray-100 pt-3 space-y-2">
                <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400 w-14 shrink-0">Khoa</span>
                    <span className="text-xs font-medium text-gray-700 truncate">{data.lecturerFaculty}</span>
                </div>
                <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400 w-14 shrink-0">Ngành</span>
                    <span className="text-xs font-medium text-gray-700 truncate">{data.lecturerMajor}</span>
                </div>
                <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400 w-14 shrink-0">Sĩ số</span>
                    <span
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-3">
            <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-900 truncate">{student.user.fullName}</p>
...
function StudentTableRow({ student }: { student: IStudentOfHomeroomClass }) {
    return (
        <div className="grid grid-cols-[2fr_1fr_1.2fr_1.5fr] gap-3 px-4 py-3 border-b border-gray-100 last:border-0 items-center hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-2.5 min-w-0">
                <Avatar avatar={student.user.avatar} fullName={student.user.fullName} size={30} />
                <span className="text-sm font-medium text-gray-800 truncate">{student.user.fullName}</span>
            </div>
            <span className="text-sm text-gray-500">{student.studentCode}</span>
            <span className="text-sm text-gray-500 truncate">{student.major.name}</span>
            <span className="text-xs text-gray-400 truncate">{student.user.email}</span>
        </div>
    );
}
    ...
    return (
        <div className="w-full space-y-4 mt-5 p-3 md:p-5">
            ...
            <div className="hidden md:block rounded-2xl border border-gray-200 bg-white overflow-hidden">
                <div className="grid grid-cols-[2fr_1fr_1.2fr_1.5fr] gap-3 px-4 py-2.5 bg-gray-50 border-b border-gray-100">
